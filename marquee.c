 ...
 * Description:
 *  This program will visibly cycle through the DELL Keyboard LEDs in one
 * direction. Notably, if you would like to reverse the direction of the strobe,
 * you may press R to have it run from right to left or L to have it run from
 * left to right.
 */
...
int main(void) {
  ...
  while (1) {
    ...
    /*For every 200 ms, update the LED output and shift the active bit in the
     * current direction. This includes a wraparound.*/
    ftime(&t);
...
static void setLeds(int ledBits) { ioctl(STDIN_FILENO, KDSETLED, ledBits); }

/*Enable or restore raw keyboard mode to allow immediate, non-echoed key
 * input.*/
static void changeMode(int dir) {
...
/*Check whether or not a key has been pressed without pausing the program
 * execution.*/
static int kbhit(void) {
  struct timeval tv;
  fd_set rdfs;
